<script lang="ts">
	...
	$effect(() => {
		...
		const blob = new Blob([data.写真.bytes as BlobPart], { type: 'image/jpeg' });
...
<section class="space-y-3">
	...
	<fieldset class="space-y-2">
		...
		{/if}
		<input type="file" accept="image/*" onchange={onPhotoSelect} class="block text-sm" />
		{#if photoError}
			<p class="text-sm text-red-700">{photoError}</p>
		{/if}
		<p class="text-xs text-gray-500">
			長辺 600px までリサイズし、JPEG（品質 85）として埋め込みます。共有リンクには含まれません。
		</p>
	</fieldset>
</section>
