...
async function renderDialog(props?: { closeOnBackdropClick?: boolean }) {
  let closes = 0;
...
describe("Dialog — native modal behavior", () => {
  it("opens itself modally on mount", async () => {
    const { dialog } = await renderDialog();
    expect(dialog.open).toBe(true);
  });

  it("is centered in the viewport, not pinned to a corner", async () => {
  ...
  it("dims the background via the scoped ::backdrop style", async () => {
  ...
  it("closes on the native cancel event (Escape)", async () => {
    const { dialog, getCloses } = await renderDialog();
  ...
  it("stays open when content inside the frame is clicked", async () => {
    const { dialog, getCloses } = await renderDialog();
